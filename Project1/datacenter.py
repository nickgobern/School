...
class DataCenter(Topo):
	...
	for m in range(fi):
		mid_level_switch = self.addSwitch('mls%s' % (m + 1))
		for l in range(fi):
			low_level_switch = self.addSwitch('s%sx%s'% (m + 1, l + 1))
			for h in range(n):
				host = self.addHost('h%sx%sx%s' % (m + 1, l + 1, h + 1))
				self.addLink(host, low_level_switch)
			self.addLink(low_level_switch, mid_level_switch)
		self.addLink(mid_level_switch, tls)
